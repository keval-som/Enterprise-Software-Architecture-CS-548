...
//			UUID tid = dto.getId();
//			URI location = uriInfo.getBaseUriBuilder().path(id).path("treatment").path(tid.toString()).build();
//			return Response.status(Status.CREATED).header("Location", location.toString()).entity(tid.toString()).build();
